...
# --- CONFIGURATION (use Path to avoid Windows-style "\" issues on Render/Linux) ---
ENGINE = None
...
def root():
    # Render (and load balancers) often ping "/" with HEAD
    return "OK", 200
...
def update_index():
    ...
    try:
        ...
        if not ppt_files:
            return (
                jsonify(
                    {"error": "no_ppt_files_found", "path": str(input_path.resolve())}
                ),
                400,
            )
